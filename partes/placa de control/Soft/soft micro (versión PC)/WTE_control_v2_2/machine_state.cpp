...
const auto PER_T_UPDATING = 400ms;
...
{
    set_wind_time(wind_str_pp, time_str_pp);
}
// //////////////////// PARA PRUEBAS //////////////////////
...
//            uint8_t time_sim_int = (uint8_t)roundf( ((*time_sim)[i_wind_current]) *1000.01 );
            ...
            if(tmr_wind_condition > time_sim_int)
            {
                ...
                if( i_wind_current >= dim_time )
                {
                    ...
                    //stop_simulation(setpoint, PER_T_UPDATING);
                    stop_simulation(setpoint);
